...
func ScanStructByFields(src map[string]interface{}, dest interface{}) error {
	d := reflect.ValueOf(dest)
	if d.Kind() != reflect.Ptr || d.IsNil() {
		return errScanStructValue
	}
	...
	for k, v := range src {
		...
		fs := ss.fieldSpec([]byte(k))
		if fs == nil {
			continue
		}
		if err := convertAssignValue(d.FieldByIndex(fs.index), v); err != nil {
			return fmt.Errorf("redigo.ScanStruct: cannot assign field %s: %v", fs.name, err)
		}
	}
	return nil
